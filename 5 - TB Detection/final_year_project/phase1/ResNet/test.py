...
MODEL_REL_PATH = Path(__file__).parent / "tb_resnet50_clean.onnx"
...
def preprocess(image_path: str) -> np.ndarray:
    ...
    # Model expects [batch, height, width, channels] (NHWC format)
...
def predict_resnet(image_path: str):
    ...
    if pred_class == 0:
        ...
        pred_class = 0
    return pred_class, probs.tolist()[::-1]  
...
if __name__ == "__main__":
    ...
    pred_res, probs_res = predict_resnet(img_path)
    print("Raw probs:", probs_res)
    print("Predicted class index:", pred_res)
    if pred_res == 0:
